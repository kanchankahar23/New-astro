    ...
    <div class="content-wrapper">
        ...
        <div class="card-body" style="background-color: #fff; border-radius:10px;margin-bottom: 11px;">
            <div class="row">
                ...
                <div class="col-md-3">
                    <h4>Total Balance : <b><span class="badge badge-success">{{ $total ?? 0.0 }}&nbsp; &#8377;</span></b>
                    </h4>
                </div>
                <div class="col-md-3">
        ...
        <form action="{{ url('allover-transection') }}" method="Post" enctype="multipart/form-data">@csrf
            <div class=" ">
                <div class="mb-3 card col-xl-12 d-flex">
                    <div class="row ">
                        ...
                        <div class="mt-4 col-xl-2 ">
                            <label>Amount</label>
                            <input type="text" class="form-control" placeholder="Search via Amount" name="amount"
                                value="{{ $amount ?? '' }}">
                        </div>
                        <div class="mt-4 col-xl-2 ">
                            <label>Pay ID</label>
                            <input type="text" class="form-control" placeholder="Search via Pay Id" name="pay_id"
                                value="{{ $pay_id ?? '' }}">
                        </div>
            ...
            <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ ++$i }}.</td>
                            <td><a
                                    href="{{ url('all-transection-user', $transection->user_id) }}">
                                    {{ ucfirst($transection->name) }}</a>
                                <a onclick="addBonus('{{ url('add-bonus', $transection->id) }}')"
                                    class="bonus-link" style="position: relative;">
                                    <i class="fa fa-user"></i>
                                    <span class="tooltip-text">Add Bonus</span>
                                </a>
                            </td>
                            <td>&#8377; &nbsp;{{ $transection->amount ?? 00.0 }} </td>
                            <td>&#8377; &nbsp;{{ $transection->bonus ?? 00.0 }}
                                @if (!empty($transection->remark))
                                <a onclick="appointmentHistory('{{ url('special-bonus' . '?id=' . $transection->id) }}')"
                                    class="bonus">Special Offer
                                </a>
                                @endif
                            </td>
                            <td>{{ $transection->order_id }}</td>
                            <td>{{ \Carbon\Carbon::parse($transection->date)->format('d M, Y')
                                }}</td>
                        </tr>
                        @endforeach
                        @if (!empty($total))
