                    ...
                    <div class="appointments-container">
                        ...
                        <div class="appointment-card">
                            <div class="card-header">
                                <div class="user-info">
                                    <img src="{{ asset($chat->userDetails->avtar ?? '' ?? 'https://randomuser.me/api/portraits/men/32.jpg') }}"
                                        alt="user" class="avatar">
                                    <div class="user-details">
                                        <span class="user-name">{{
                                            ucwords($chat->userDetails->name ?? '')
                                            }}</span>
                            ...
                            <div class="card-footer">
                                ...
                                <div>
                                    ...
                                    {{--  <span style="cursor: pointer;" class="accept start-chat-btn status approved"
                                        data-astro="{{ $chat->user_encrypt }}" data-chat-id="{{ $chat->id }}"
                                        style="width: max-content;">Start
                                        Chat</span>  --}}
                                        <div data-user-id="{{ $chat->user_id }}">
                                            <span class="user-busy-status"></span>
                                        </div>
                                        <div class="start-chat-btn" data-user-id="{{ $chat->user_id }}">
                                    <span style="cursor: pointer;" class="accept start-chat-btn status approved"
                                        data-astro="{{ $chat->user_encrypt}}" data-chat-id="{{ $chat->id }}"
                                        style="width: max-content;">Start
                                        Chat</span>
                        ...
                        {{--  <div class="appointment-card">
                            <div class="card-header">
                                <div class="user-info">
                                    ...
                                    <div class="user-details">
                                        ...
                                        <span class="user-role">
                                            @if(!empty($chat->userDetails->last_seen)>=
                                            now()->subMinutes(5) &&
                                            $chat->userDetails->active_status == 1)
                                            <span class="badge badge-success circle">
                                                Online
                                            </span>
                                            @elseif(!empty($chat->userDetails->active_status) == 2)
                                    ...
                                    @elseif($chat->is_complete == 2)
                                    <span style="cursor: pointer;" class="accept start-chat-btn status approved"
                                        data-astro="{{ $chat->user_encrypt }}"
                                        data-chat-id="{{ $chat->id }}" style="width: max-content;">Start
                                        Chat</span>
                                    @endif
                                </div>
                            ...
                            <div class="card-footer">
                                ...
                                <div class="time-info status pending time-badge">
                                    ...
                                    <div class="time-part">
                                        <i class="fa fa-indian-rupee-sign" aria-hidden="true" style="color: white;"></i>&nbsp;
                                        {{ $chat->wallet ?? '' }}
                                    </div>
                                </div>
                            </div>
            ...
            <div class="tabs__panel" id="tab-panel-2" style="">
                <div class="tabs__panel-image">
                    <div class="appointments-container">
                        ...
                        <div class="appointment-card">
                            <div class="card-header">
                                <div class="user-info">
                                    ...
                                    @elseif(!empty($chat->userDetails->active_status)
                                    == 2)
                                    <span class="status" style="background-color: #4ea615;">
                                        Busy</span>
                                    @else
                        ...
                        @else
                        {{-- <a
                            href="{{ url('chat-details-user',$chat->astrologerDetails->id ) }}"
                            </a>
                            --}}
...
<script>
    function gotoChat(id){
        startChats(id);
        console.log(id);
       window.open("{{ url('real-time-chat') }}" +"/" + id);
    }
