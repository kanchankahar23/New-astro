                        ...
                        <div class="row g-0">
                            <div class="mx-auto text-center col-10 col-md-9">
                                ...
                                <form id="loginForm" class="form-border" method="post">
                                    ...
                                    <div class="mb-3">
                                        <input type="password" placeholder="Password" id="password" onkeyup="checkInputValues()" class="form-control">
                                    </div>
                                    <div class="mb-3">
                                        <input type="password" placeholder="Confirm Password" id="cpassword" onkeyup="checkInputValues()" class="form-control">
                                    </div>

                                    <div class="my-4 d-grid">
                                        <button class="btn btn-primary text-uppercase button" type="button" id='submitBtn' onclick="register()" disabled style="background-color:grey;">
                                            Save Password
                                          </button>
                                    </div>
                                </form>
                                <p class="text-2">Already User in Astro-Buddy? <a href="{{ url('/login') }}">Login</a></p>
                            </div>
                        </div>
                    </div>
...
">
    ...
    <script>
       ...
       function checkInputValues() {
           ...
           const input2 = document.getElementById('cpassword').value;
       ...
       function register(){
           try {
               var details = document.getElementsByTagName('input');
               // console.log(details[0].value);
               // console.log(details[1].value);
               const apiUrl = "{{ url('api/register-password-save') }}"; // Replace this with your actual API URL

               // Data to be sent in the request body
               const postData = {
                    user_password:details[0].value,
                   _token: "{{ csrf_token() }}",
               };
